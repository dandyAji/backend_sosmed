...
export const updateAvatar = async (req, res) => {
  try {
    ...
    // validasi fungsi untuk hapus gambar lama
    if (currentUser.imageId) {
      // hapus gambar lama dengan imageId dari cloudinary
    ...
    // update user dengan imageId baru di database
    const updatedUser = await prisma.user.update({
      where: {
        id: req.user.id,
      },
    ...
    console.log(error);
    res.status(500).json({
      message: "terjadi kesalahan pada server2s",
      data: error,
    });
  }
};
